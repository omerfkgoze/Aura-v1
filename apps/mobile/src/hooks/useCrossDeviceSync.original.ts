...
export const useCrossDeviceSync = (): UseCrossDeviceSyncReturn => {
  ...
  const refreshSyncStatus = useCallback(async () => {
    try {
      const status = await syncManager.refreshSyncStatus();

      setSyncState(prev => ({
        ...prev,
        syncStatus: status.success ? 'synchronized' : 'out_of_sync',
        connectedDevices: status.syncedDevices || [],
        offlineDevices: status.failedDevices || [],
        pendingRotations: 0,
        lastSyncTime: status.timestamp,
        conflictsDetected: status.conflicts?.length || 0,
      }));

      // Check for conflicts
      if (status.conflicts && status.conflicts.length > 0) {
        setDetectedConflicts(status.conflicts);
      }
  ...
  }, [isInitialized, deviceId, refreshSyncStatus]);
  ...
  }, [isInitializing, isOnline, refreshSyncStatus]);
