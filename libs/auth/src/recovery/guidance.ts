 ...
 */
export function getRecoveryPhraseGuidance(): RecoveryGuidance {
  return {
 ...
 */
export function getShamirSharingGuidance(
  totalShares: number,
  threshold: number
): {
  distribution: string[];
...
export function getRecoverySecurityChecklist(): {
  setup: { task: string; critical: boolean }[];
  storage: { task: string; critical: boolean }[];
  testing: { task: string; critical: boolean }[];
  maintenance: { task: string; critical: boolean }[];
} {
  return {
    setup: [
...
export function getPersonalizedRecoveryStrategy(profile: {
  ...
  const familyStrategy = strategies.family[profile.hasFamily.toString() as 'true' | 'false'];
  const techStrategy = strategies.techSavvy[profile.techSavvy.toString() as 'true' | 'false'];
  const travelStrategy = strategies.travel[profile.travels.toString() as 'true' | 'false'];
...
export function getRecoverySetupGuide(): {
  steps: { step: number; title: string; description: string; actions: string[] }[];
  tips: string[];
