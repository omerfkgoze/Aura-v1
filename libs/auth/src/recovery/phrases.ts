...
// BIP39 wordlist (English) - first 100 words for demo
// In production, use the complete 2048-word BIP39 wordlist
...
async function entropyToMnemonic(
  ...
  for (let i = 0; i < fullBinary.length; i += 11) {
    ...
    if (index >= BIP39_WORDLIST.length) {
      // In production, this should use the full 2048-word BIP39 wordlist
      throw new Error(
        `Word index ${index} exceeds wordlist length. Use full BIP39 wordlist in production.`
      );
    }
