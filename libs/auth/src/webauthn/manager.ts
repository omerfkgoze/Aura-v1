...
import type { SecureStorage, SecureStorageConfig } from './storage';
...
export class WebAuthnManager {
  private registration: WebAuthnRegistration;
  private authentication: WebAuthnAuthentication;
  ...
  // Registration Flow
  async startRegistration(request: WebAuthnRegistrationRequest): Promise<{
    options: any;
    challenge: string;
  }> {
    try {
  ...
  // Authentication Flow
  async startAuthentication(request: WebAuthnAuthenticationRequest): Promise<{
    options: any;
    challenge: string;
  }> {
    try {
      ...
      let allowedCredentials: WebAuthnCredential[] = [];
  ...
  async startConditionalAuthentication(): Promise<any> {
    try {
      ...
      const request: WebAuthnAuthenticationRequest = {
  ...
  private async getCredential(credentialId: string): Promise<WebAuthnCredential | null> {
    try {
  ...
  private async retrieveChallenge(userId?: string): Promise<string | null> {
    try {
      const challengeKey = userId ? `challenge:${userId}` : 'challenge:anonymous';
      ...
      const challenge: WebAuthnChallenge = JSON.parse(challengeData);
  ...
  private async removeChallenge(userId?: string): Promise<void> {
    const challengeKey = userId ? `challenge:${userId}` : 'challenge:anonymous';
