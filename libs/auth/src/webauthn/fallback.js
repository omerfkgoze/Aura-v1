...
export class FallbackAuthenticationManager {
  ...
  constructor(rpId, rpName) {
    ...
    const recoveryConfig = {
      storage: {
        storeRecoveryPhrase: async (_userId, _phrase) => {},
        getRecoveryPhrase: async _userId => null,
        storeShamirShares: async (_userId, _shares) => {},
        getShamirShares: async _userId => [],
        storeEmergencyCode: async _code => {},
        validateEmergencyCode: async (_codeId, _code) => null,
        markCodeAsUsed: async (_codeId, _usedFromIp) => {},
   ...
   */
  async authenticateWithFallback(identifier, options) {
    const config = {
   ...
   */
  async registerWithFallback(userId, username, displayName, options) {
    const config = {
      skipMethods: options?.skipMethods || [],
   ...
   */
  async testMethodAvailability(methodName) {
  ...
  async isMethodSupported(method, capabilities) {
  ...
  async attemptMethod(method, identifier, _capabilities) {
    switch (method.handler) {
      case 'webauthn-platform':
      case 'webauthn-roaming':
  ...
  async attemptRegistration(method, userId, username, _displayName, _capabilities) {
    switch (method.handler) {
      case 'webauthn-platform':
      case 'webauthn-roaming':
        // This would integrate with the unified WebAuthn manager
  ...
  async getRecoveryOptions(capabilities) {
    const options = [];
