...
export class TokenManager {
  ...
  async verifyAccessToken(token: string): Promise<TokenPayload> {
    try {
      const payload = (await this.verifyToken(token, this.JWT_SECRET)) as TokenPayload;
  ...
  async verifyRefreshToken(token: string): Promise<RefreshTokenPayload> {
    try {
      const payload = (await this.verifyToken(token, this.REFRESH_SECRET)) as RefreshTokenPayload;
  ...
  async refreshSession(refreshToken: string, user: User): Promise<AuthSession> {
  ...
  private async verifyToken(token: string, secret: string): Promise<object> {
