...
export class TokenManager {
  ...
  createSession(user, deviceId, authMethod) {
    return __awaiter(this, void 0, void 0, function* () {
      const now = new Date();
      const sessionId = yield this.generateSessionId();
      ...
      const accessToken = yield this.signToken(accessTokenPayload, this.JWT_SECRET);
      const refreshToken = yield this.signToken(refreshTokenPayload, this.REFRESH_SECRET);
      return {
        accessToken,
        refreshToken,
  ...
  verifyAccessToken(token) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        const payload = yield this.verifyToken(token, this.JWT_SECRET);
  ...
  verifyRefreshToken(token) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        const payload = yield this.verifyToken(token, this.REFRESH_SECRET);
        // Check if token is expired
        const now = Math.floor(Date.now() / 1000);
        if (payload.exp < now) {
  ...
  refreshSession(refreshToken, user) {
    return __awaiter(this, void 0, void 0, function* () {
      const refreshPayload = yield this.verifyRefreshToken(refreshToken);
  ...
  signToken(payload, secret) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const signature = yield this.sign(`${encodedHeader}.${encodedPayload}`, secret);
  ...
  verifyToken(token, secret) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      // Verify signature
      const expectedSignature = yield this.sign(`${encodedHeader}.${encodedPayload}`, secret);
  ...
  sign(data, secret) {
    return __awaiter(this, void 0, void 0, function* () {
      if (typeof crypto !== 'undefined' && crypto.subtle) {
        ...
        const signature = yield crypto.subtle.sign('HMAC', key, new TextEncoder().encode(data));
